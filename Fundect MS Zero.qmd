...
def fix_geoms(gdf):
  ...
  # MELHORIA: INCLUIR RETURN e PRINT COM QTDE DE REGISTROS LIDOS, CORRIGIDOS, E INVÁLIDOS.
...
# MELHORIA: Incluir a diferença de área por recálculo na Tabela 2 do relatório, e explicar porque foi necessário.
...
# MELHORIA: Incluir na tabela 2 as diferenças em área e emissões por conta da intercessão com ZEE-MS.
```

```{python}
#Conferindo totais:
MS_ZEE_gdf = gpd.read_file(r"Resultados\MS_ZEE_gdf.gpkg", engine="pyogrio", use_arrow=True)

fixed_MS_products_gdf = gpd.read_file(r"Resultados\fixed_MS_products_gdf.gpkg", engine="pyogrio", use_arrow=True)

MS_products_gdf = gpd.read_file(r"Resultados\MS_products_gdf.gpkg", engine="pyogrio", use_arrow=True)

MS_ZEE_products_gdf = gpd.read_file(r"Resultados\MS_ZEE_products_gdf.gpkg", engine="pyogrio", use_arrow=True)
```
```{python}
if verbose:
...
```{python}
def plot_boxplot_grid(df, lo = 'lo', hi = 'hi', x_label_func=ipcc_label, y_label= "Fatores de emissão (ton Co2eq/ha/ano)", y_fontsize = 12, x_fontsize = 12, y_tick_fontsize = 12, title_font_size = 12):
  """
...
```{python}
# Figura 2
MS_ZEE_1994_2016_emissions_df = pd.read_csv(r"Resultados\Emissoes\MS_ZEE_1994_2016_emissions_df.csv")
...
```{python}
# Figura 3
emissions_2001_2021_total_df = pd.read_csv(r"Resultados\Emissoes\emissions_2001_2021_total_df.csv")

plot_boxplot_grid(emissions_2001_2021_total_df, lo='min', hi='max', y_label = "Emissões (ton CO2eq/ano)", x_fontsize=16, y_fontsize=16, y_tick_fontsize = 16, title_font_size = 16)
```

### Áreas que permaneceram em Floresta ou Campo e Pastagem com emissões positivas
...
```{python}
fe_df = MS_ZEE_1994_2016_emissions_df
fe_df = fe_df[fe_df['ipccgr_ini'] == fe_df['ipccgr_fin']].drop(['area_ha', 'rows'], axis=1)

df = fe_df.merge(gdf, on=['SIGLA', 'ipccgr_ini'])

print(df[df['el']>df['q3']].groupby(['SIGLA', 'ipccgr_ini', 'cagr_ini', 'cagr_fin'])[['el']].sum().reset_index())
```

### Imprimindo as mudanças de uso da terra por ZEE

```{python}
# Tabela 3: Mudanças de uso da terra por ZEE do MS para o período 2001-2021, em ha. Fonte: Os autores.
...
### Comparando emissões médias anuais do Quarto Inventário e nossa estimativa de 2001-2010

```{python}
# Médias anuais do Quarto inventário, em todos os períodos.

gdf = gpd.read_file(r"Resultados\MS_ZEE_products_gdf.gpkg", engine="pyogrio", use_arrow=True)
```

```{python}
print('gdf.el',  "{:.3e}".format((gdf['el_9402'].sum() + gdf['el_0210'].sum() + gdf['el_1016'].sum())/(2016-2010+2010-2002+2002-1994)))
```

```{python}
# Médias anuais de 2001 a 2021.

df2 = pd.read_csv(r"Resultados\Emissoes\emissions_2001_2021_total_df.csv")
#df2 = df2.drop(['min', 'lo', 'hi', 'max'], axis=1)

print(df2.sum(numeric_only=True).map('{:.3e}'.format))
```
